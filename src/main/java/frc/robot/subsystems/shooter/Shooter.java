...
public class Shooter extends SubsystemBase {
  ...
  private final DoubleSupplier mHubDistanceSupplier;
  ...
  public Shooter(DoubleSupplier hubDistanceSupplier) {
    mHubDistanceSupplier = hubDistanceSupplier;
    ...
    mKickerLeader =
        new SparkMax(ShooterConstants.SHOOTER_FEEDER_LEADER_CANID, MotorType.kBrushless);
    mKickerFollower =
        new SparkMax(ShooterConstants.SHOOTER_FEEDER_FOLLOWER_CANID, MotorType.kBrushless);

    mShooterLeader.configure(
        ShooterConfig.shooterLeaderConfig,
        ResetMode.kResetSafeParameters,
        PersistMode.kPersistParameters);
    mShooterFollower.configure(
        ShooterConfig.shooterFollowerConfig,
        ResetMode.kResetSafeParameters,
        PersistMode.kPersistParameters);
    mKickerLeader.configure(
        ShooterConfig.shooterFeederLeaderConfig,
        ResetMode.kResetSafeParameters,
        PersistMode.kPersistParameters);
    mKickerFollower.configure(
        ShooterConfig.shooterFeederFollowerConfig,
        ResetMode.kResetSafeParameters,
        PersistMode.kPersistParameters);
    ...
    mShooterPID =
        new PIDController(
            ShooterConstants.LEADER_Kp, ShooterConstants.LEADER_Ki, ShooterConstants.LEADER_Kd);

    tempFF =
        new SimpleMotorFeedforward(
            ShooterConstants.LEADER_FF_kS,
                  ...
                  log.motor("shooter-leader")
                      .voltage(
                          m_appliedVoltage.mut_replace(
                              mShooterLeader.getBusVoltage() * mShooterLeader.getAppliedOutput(),
                              Volts))
                      .angularPosition(
                          m_encoderAngle.mut_replace(
                              mShooterLeaderEncoder.getPosition(), Rotations))
                      .angularVelocity(
                          m_encoderVelocity.mut_replace(
                              // Encoder reports RPM – convert to RPS for WPILib units
                              mShooterLeaderEncoder.getVelocity() / 60.0, RotationsPerSecond));
                },
  ...
  /**
   * Compute a target RPM from a polynomial regression based on distance and update the internal
   * target RPM. Falls back to a safe RPM if the regression suggests an unrealistically low value.
   *
  ...
  /**
   * Compute required wheel RPM from kinematic projectile motion equations and update the target
   * RPM. Falls back for invalid geometry.
   *
  ...
  /**
   * Whether the flywheel is within tolerance of the current target RPM (including offset).
   *
   * @return true if at speed
   */
  public boolean isAtSpeed() {
    return Math.abs(mShooterLeaderEncoder.getVelocity() - (mTargetRPM + mShooterRPMOffset))
  ...
  /**
   * Return the motor output computed by the PID+feedforward controller without applying it.
   *
  ...
  /**
   * Spin up to the current (continuously updated) target RPM and wait until within tolerance before
   * feeding the kicker. Cleans up (disables shooter, stops kicker) when interrupted.
   */
  public Command shootWhileHeldCommand() {
    return Commands.sequence(
            runOnce(() -> mShooterEnabled = true),
            Commands.waitUntil(this::isAtSpeed),
            run(() -> runKicker(-ShooterConstants.KICKER_SPEED)))
        .finallyDo(
            () -> {
  ...
  /**
   * Manual override: spin up and feed immediately, bypassing the at-speed gate. For use if the
   * distance-based target RPM can't be trusted and the driver wants to force a shot.
   */
  ...
  public Command decreaseShootingRPMOffsetCommand() {
    return new InstantCommand(
        () -> changeShootingRPMOffset(-ShooterConstants.RPM_OFFSET_INCREMENT));
  }
  ...
  /**
   * Dynamic test – applies a voltage step. Used to fit kA.
   *
